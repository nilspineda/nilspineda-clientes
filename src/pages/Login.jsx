...
export default function Login() {
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true);
    try {
      const authData = await signIn(email, password, mfaRequired ? mfaOtp : undefined);
      if (mfaRequired) {
      ...
      if (!mfaRequired && err?.response?.data?.mfa?.enabled) {
        setMfaRequired(true);
        setMfaOtp("");
        setError("");
      } else {
                ...
                {mfaRequired ? (
                  <>
                    <div className="bg-primary/5 border border-primary/20 rounded-lg p-4 text-center">
                      <p className="text-sm text-foreground font-medium">Verificación en dos pasos</p>
                      <p className="text-xs text-muted-foreground mt-1">
                        Ingresa el código enviado a <span className="font-medium">{email}</span>
                      </p>
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="mfa-otp">Código de verificación</Label>
                      <Input
                        id="mfa-otp"
                        type="text"
                        inputMode="numeric"
                    ...
                    <Button type="submit" disabled={loading || mfaOtp.length < 4} className="w-full">
                      {loading ? (
                        <>
