...
export default function AdminPayments() {
  ...
  async function fetchData() {
    const [paymentsRes, usersRes, userServicesRes] = await Promise.all([
      supabase.from('payments').select('*, user_services(name, url_dominio), profiles(name, whatsapp)').order('payment_date', { ascending: false }),
      supabase.from('profiles').select('id, name').order('name', { ascending: true }),
      supabase.from('user_services').select('id, name, url_dominio, user_id, services(name)').order('created_at', { ascending: false })
    ])
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Total: ${totalAmount.toLocaleString()}</p>
          </div>
        </div>
        <button
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <select
              ...
              onChange={e => {
                setFormData({ ...formData, user_id: e.target.value, service_id: '' })
              }}
              required
              ...
              >
                ...
                {getServicesForUser(formData.user_id).map(s => (
                  <option key={s.id} value={s.id}>
                    {s.name || s.services?.name} {s.url_dominio ? `(${s.url_dominio})` : ''}
                  </option>
                ))}
              </select>
            </div>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        {payment.profiles?.whatsapp && (
                          (() => {
                            const wa = normalizeWhatsapp(payment.profiles.whatsapp)
                            return (
                              <a
                                href={`https://wa.me/${wa}`}
                            target="_blank"
                        ...
                        <span className="font-semibold text-foreground">{payment.profiles?.name}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-muted-foreground">
                      {payment.user_services?.name || payment.user_services?.services?.name || '-'}
                      {payment.user_services?.url_dominio && (
                        <span className="block text-xs text-blue-400">{payment.user_services.url_dominio}</span>
                      )}
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <select
                        ...
                        className={`text-sm border rounded-lg px-3 py-1.5 ${
                          payment.status === 'paid' ? 'bg-green-500/10 border-green-500/30 text-green-400' :
                          payment.status === 'pending' ? 'bg-yellow-500/10 border-yellow-500/30 text-yellow-400' :
