  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-2xl bg-green-500/20 flex items-center justify-center">
            <svg className="w-6 h-6 text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
          </div>
          <div>
      ...
      <div className="flex gap-2 flex-wrap">
        <button onClick={exportCSV} className="px-4 py-2 rounded-xl bg-muted border border-border text-muted-foreground hover:text-foreground hover:bg-accent text-sm transition-all">Exportar CSV</button>
        <button onClick={() => setShowModal(true)} className="px-4 py-2 rounded-xl bg-primary text-foreground hover:bg-primary-light text-sm font-medium transition-all">Registrar Pago</button>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-card rounded-2xl border border-border p-5">
          <p className="text-sm text-muted-foreground">{currentMonth}</p>
          <p className="text-3xl font-bold text-green-400">{formatCurrency(stats.thisMonth)}</p>
        </div>
        <div className="bg-card rounded-2xl border border-border p-5">
      ...
      <div className="bg-card rounded-2xl border border-border p-4">
        <div className="flex flex-wrap gap-4 items-end">
          <input type="text" placeholder="Buscar cliente..." value={searchClient} onChange={(e) => setSearchClient(e.target.value)} className="px-4 py-2 bg-muted border border-border rounded-xl text-foreground text-sm w-full sm:w-48" />
          <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="px-4 py-2 bg-muted border border-border rounded-xl text-foreground text-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className="px-3 py-2 bg-muted border border-border rounded-xl text-foreground text-sm" />
          </div>
          <div className="flex items-center gap-2">
            <label className="text-sm text-muted-foreground">Hasta:</label>
            <input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="px-3 py-2 bg-muted border border-border rounded-xl text-foreground text-sm" />
          </div>
          {(filterStatus !== "all" || dateFrom || dateTo) && (
            <button onClick={() => { setFilterStatus("all"); setDateFrom(""); setDateTo(""); }} className="px-3 py-2 text-sm text-muted-foreground hover:text-foreground">Limpiar filtros</button>
          )}
        </div>
      </div>
      ...
      <Modal isOpen={showModal} onClose={resetForm} title={editingPayment ? "Editar Pago" : "Registrar Pago"} size="md">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <select value={formData.user_id} onChange={(e) => setFormData({ ...formData, user_id: e.target.value, service_id: "" })} required className="w-full px-4 py-3 bg-muted border border-border rounded-xl text-foreground focus:ring-2 focus:ring-primary focus:border-transparent outline-none">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" placeholder="0" value={formData.amount} onChange={(e) => setFormData({ ...formData, amount: e.target.value })} required className="w-full px-4 py-3 bg-muted border border-border rounded-xl text-foreground placeholder-muted-foreground focus:ring-2 focus:ring-primary focus:border-transparent outline-none" />
            </div>
            <div>
              <label className="block text-sm font-medium text-muted-foreground mb-2">Fecha</label>
              <input type="date" value={formData.payment_date} onChange={(e) => setFormData({ ...formData, payment_date: e.target.value })} className="w-full px-4 py-3 bg-muted border border-border rounded-xl text-foreground focus:ring-2 focus:ring-primary focus:border-transparent outline-none" />
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium text-muted-foreground mb-2">Método de pago</label>
            <select value={formData.payment_method} onChange={(e) => setFormData({ ...formData, payment_method: e.target.value })} className="w-full px-4 py-3 bg-muted border border-border rounded-xl text-foreground focus:ring-2 focus:ring-primary focus:border-transparent outline-none">
          ...
          <div className="flex gap-3 pt-4">
            <button type="submit" className="flex-1 bg-primary text-foreground px-6 py-3 rounded-xl hover:bg-primary-light transition-colors font-medium">{editingPayment ? "Guardar Cambios" : "Registrar Pago"}</button>
            ...
            <tbody className="divide-y divide-border-dark">
              {filteredPayments.length === 0 ? (
                ...
                filteredPayments.map((payment) => (
                  <tr key={payment.id} className="hover:bg-gradient-to-r hover:from-primary/5 hover:to-transparent transition-all">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        {payment.expand?.user_id?.whatsapp && (
                          <a href={`https://wa.me/${normalizeWhatsapp(payment.expand.user_id.whatsapp)}`} target="_blank" rel="noopener noreferrer" className="w-8 h-8 rounded-lg bg-green-500/20 flex items-center justify-center hover:bg-green-500/30 transition-colors">
                    ...
                    <td className="px-6 py-4 text-muted-foreground">
                      ...
                      {payment.expand?.user_service_id?.url_dominio && <span className="block text-xs text-blue-400">{payment.expand?.user_service_id?.url_dominio}</span>}
                    </td>
                    <td className="px-6 py-4"><span className="px-3 py-1.5 rounded-xl bg-green-500/10 border border-green-500/20 text-green-400 font-bold">{formatCurrency(payment.amount)}</span></td>
                    <td className="px-6 py-4 text-muted-foreground text-sm">{formatDate(payment.payment_date)}</td>
                    <td className="px-6 py-4"><span className="px-2 py-1 rounded-lg text-xs bg-muted text-muted-foreground capitalize">{payment.payment_method}</span></td>
                    <td className="px-6 py-4">
                      <select value={payment.status} onChange={(e) => updateStatus(payment.id, e.target.value)} className={`text-sm border rounded-lg px-3 py-1.5 ${payment.status === "paid" ? "bg-green-500/10 border-green-500/30 text-green-400" : payment.status === "pending" ? "bg-yellow-500/10 border-yellow-500/30 text-yellow-400" : "bg-red-500/10 border-red-500/30 text-red-400"}`}>
                        <option value="paid">Pagado</option>
                        <option value="pending">Pendiente</option>
                        <option value="failed">Fallido</option>
                      </select>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <button onClick={() => handleEdit(payment)} className="px-3 py-1.5 rounded-xl bg-primary/10 hover:bg-primary text-primary hover:text-foreground font-medium text-sm transition-all">Editar</button>
                        <button onClick={() => handleDelete(payment.id)} className="px-3 py-1.5 rounded-xl bg-red-500/10 hover:bg-red-500 text-red-400 hover:text-foreground font-medium text-sm transition-all">Eliminar</button>
                      </div>
                    </td>
                  </tr>
                ))
