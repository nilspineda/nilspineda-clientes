    ...
    return (
      <div className="min-h-[80vh] flex items-center justify-center">
        <div className="text-center max-w-md mx-auto p-8">
          ...
          <a
            href={`https://wa.me/${normalizeWhatsapp(whatsappNumber || "3167195500")}?text=Hola, mi cuenta aparece suspendida. Necesito ayuda.`}
            target="_blank"
          ...
          >
            ...
            {/* Últimos Pagos movidos a /payments */}
              />
            </svg>
          }
        >
          {services.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 text-sm mt-1">
                Contacta al administrador para agregar servicios
              </p>
            </div>
          ) : (
                  ...
                  <div className="relative flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <StatusBadge
                          status={
                            getDaysRemaining(service.expires_at) < 0
                              ? "expired"
                              : getDaysRemaining(service.expires_at) <= 5
                                ? "warning"
                                : getDaysRemaining(service.expires_at) === null
                                  ? "pending"
                                  : "active"
                          }
                        />
            ...
            <div className="bg-card rounded-3xl border border-border w-full max-w-md p-8">
              ...
              <form onSubmit={handleUpdateProfile} className="space-y-4">
                ...
                <button
                  ...
                  className="w-full py-3 bg-primary hover:bg-primary-dark text-white rounded-xl font-semibold transition-all disabled:opacity-50"
                >
...
function StatusBadge({ status }) {
  ...
  return (
    <span
      className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${styles[status] || styles.pending}`}
    >
...
function InfoItem({ label, value, icon, color, href }) {
  ...
  const content = (
    ...
    >
      <div
        className={`w-12 h-12 rounded-xl ${style.bg} flex items-center justify-center group-hover:scale-110 transition-transform`}
      >
        <svg
