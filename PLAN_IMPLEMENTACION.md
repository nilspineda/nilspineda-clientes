...
- Cliente deshabilitado (status=suspended) ve pantalla de bloqueo con mensaje + botón WhatsApp al admin
...
| **Tabla completa** | Todos los servicios con filtros (estado, búsqueda) |
| **Click en cliente** | Abre modal con detalles del cliente |
| **Quick actions** | Nuevo Cliente, Nuevo Servicio, Registrar Pago |
...
- Cards o tabla con:
  ...
  - Botón editar → modal: nombre, WhatsApp

### Modal de Cliente (expandido)

Al hacer click en un cliente → Vista expandida con:
- **Servicios del cliente**: Lista de servicios asignados a este cliente
  - Nombre, precio, fecha vencimiento
  - Badge estado (active/pending)
  - Badge "quién paga" (Lo administro / Cliente paga)
  - Botón editar, eliminar, ver accesos
  - Editor lexical para guardar accesos (urls, credenciales, notas técnicas)
- **Botón "Añadir Servicio"**: Formulario para agregar servicio directamente
- **Accesos**: Editor lexical general del cliente
- **Pagos**: Historial de pagos del cliente
- **Contacto**: WhatsApp directo, email
...
- Botón eliminar
- Botón ver accesos (abre modal con AccessEditor)
...
- Campos: nombre, tipo (dominio/hosting/correo/membresía/personalizado)
- Lista con botón editar y eliminar
- Muestra cuántos clientes tienen asignado cada servicio
...
- Filtros: por estado (pagado/pendiente), por rango de fechas, búsqueda por cliente
...
- Eliminar pago
- Cambiar estado inline (paid/pending/failed)
- Exportar a CSV
...
- Header: nombre, estado cuenta (activos/pendientes)
- Servicios contratados en cards:
  - Nombre, precio, fecha renovación, días restantes
  - Badge estado calculado por fecha
  - Badge quién paga
...
- Si status=suspended → pantalla de bloqueo
...
| service_id | uuid | FK services (opcional, para categorizar) |
...
-- 2. Agregar columnas a services (ya tiene: id, name, created_at)
...
| `/admin/users` | AdminUsers | Gestión de clientes (ver/editar servicios, accesos, pagos) |
...
| Admin Users | ✅ Implementado | - |
| Admin Services | ✅ Implementado | - |
| Admin Payments | ✅ Implementado | - |
| Client Dashboard | ✅ Implementado | - |
| Pantalla Suspendido | ✅ Implementado | - |
| Enviar Credenciales | ✅ Implementado | - |
| Accesos por Servicio | ✅ Implementado | - |
| Editar Perfil Cliente | ✅ Implementado | - |
| Badges Quién Paga | ✅ Implementado | - |
| Días Restantes | ✅ Implementado | - |
| Schema DB | ✅ SQL listo | - |
...
4. Filtros: estado (active/pending) y búsqueda
5. Click en cliente abre vista expandida con tabs
6. Editor lexical es privado solo para admin
7. Los servicios se asignan directamente en la vista de cada cliente
8. "Quién paga" se define al asignar servicio a cada cliente, no en el servicio base